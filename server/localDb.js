...
async function seed() {
  ...
  writeAll(defaults);
  console.log('✅ Local DB seeded with default accounts');
}
...
const User = {
  async findOne(query) {
    ...
    u.save = async function() {
      ...
      if (idx !== -1) { all[idx] = { ...this }; delete all[idx].matchPassword; delete all[idx].save; writeAll(all); }
      return this;
    };
    return u;
  },
  ...
  async find(query = {}) {
    let users = readAll();
    if (query.$or) {
      users = users.filter(u => query.$or.some(cond =>
  ...
  async updateMany(query, updates) {
    ...
    if (query._id?.$in) {
      users = users.map(u =>
        query._id.$in.includes(u._id) ? { ...u, ...updates, updatedAt: new Date().toISOString() } : u
      );
    }
    writeAll(users);
  },
