...
export default function App() {
  // ── Classes data (persisted so user edits survive reload) ──
  ...
  const handleVideoEnd = useCallback(() => {
    ...
    setWatchedIds(prev => {
      ...
      if (!already) { setWatchStats(s => ({ ...s, totalWatched: next.length })); recordStreak(); }
  ...
  const markAllWatched = () => {
    ...
    setWatchedIds(prev => { const merged = [...new Set([...prev, ...ids])]; setWatchStats(s => ({ ...s, totalWatched: merged.length })); return merged; });
  ...
  const removeCW = (plId) => {
    setContinueWatching(prev => prev.filter(x => x.plId !== plId));
    toast('Removed', 'info');
  };
  ...
  const closeModals = () => {
    ...
    setShowGlobalSearch(false); setShowSleepMenu(false); setShowSpeedMenu(false); setShowQueue(false);
  };
  ...
  return (
    <div className={`app${focusMode ? ' focus-mode' : ''}`}>
      ...
      {error && (
        <div className="error-box">
          ...
          <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
            {currentPlId && (
              <button className="err-retry" onClick={() => fetchPlaylist(currentPlId)}>🔄 Retry</button>
            )}
      ...
      {view === 'home' && (
        <main className="page">
          <div className="hero">
            ...
            {streak > 0 && <div className="streak-hero">🔥 {streak} day learning streak — keep it up!</div>}
            <input
        ...
        <main className="page">
          ...
          <div className="subject-grid">
            {selectedClass.subjects.map(sub => {
              const cwEntry = continueWatching.find(x => x.plId === sub.id);
              const watchedCount = watchedIds.filter(id => {
                // approximate: count watched IDs that were seen while this subject was active
                return cwEntry || false;
              }).length;
              return (
                <button
                  key={sub.name}
                    ...
                    {watchStats.subjectsCovered.includes(sub.name) && !completedSubjects.includes(sub.name) && (
                      <div className="sub-progress-wrap">
                        <div className="sub-progress-track">
                          <div className="sub-progress-fill" style={{ width: '50%' }} />
                        </div>
        ...
        <main className={`player-layout${focusMode ? ' player-focus' : ''}`}>
          <section className="player-col">
            <VideoPlayer
              ...
              shareVideo={shareVideo} saveNote={saveNote} exportNotes={exportNotes}
      ...
      <Modals        showShortcuts={showShortcuts} showStats={showStats}
        ...
        setWatchStats={setWatchStats} setStreakDays={setStreakDays} setCompletedSubjects={setCompletedSubjects}
      ...
      {showManage && (
        <ManagePanel
          ...
          userRole={user?.role}
          onSave={(updated) => { setClassesData(updated); setShowManage(false); }}
      ...
      <footer className="footer">
        EduConnect © 2026 — Suct EduConnect | Samacheer Kalvi Class 1–12
        <span className="footer-sep">|</span>
        <button className="footer-link" onClick={() => setShowStats(true)}>📊 Stats</button>
        <span className="footer-sep">|</span>
        <button className="footer-link" onClick={() => setTheme(t => t === 'dark' ? 'light' : 'dark')}>
